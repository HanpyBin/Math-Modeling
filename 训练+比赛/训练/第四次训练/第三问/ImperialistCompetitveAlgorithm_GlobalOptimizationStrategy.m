...
ProblemParams.NPar = 2;                           % Number of optimization variables of your objective function. "NPar" is the dimention of the optimization problem.
ProblemParams.VarMin = -5;                         % Lower limit of the optimization parameters. You can state the limit in two ways. 1)   2)
ProblemParams.VarMax = 5;                       % Lower limit of the optimization parameters. You can state the limit in two ways. 1)   2)
...
if isempty(ProblemParams.CostFuncExtraParams)
    [InitialCost, InitialOrds] = feval(ProblemParams.CostFuncName,InitialCountries);    
else
...
InitialOrds = InitialOrds(SortInd,:);
Empires = CreateInitialEmpires(InitialCountries,InitialCost,AlgorithmParams, ProblemParams, InitialOrds);
...
for Decade = 1:AlgorithmParams.NumOfDecades
    ...
    for ii = 1:numel(Empires)
        ...
        %% New Cost Evaluation
        if isempty(ProblemParams.CostFuncExtraParams)
            [Empires(ii).ColoniesCost, Empires(ii).ColoniesOrds] = feval(ProblemParams.CostFuncName,Empires(ii).ColoniesPosition);
