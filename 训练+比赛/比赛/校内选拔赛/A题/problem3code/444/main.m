...
[~,previous_croute3]=completeroute(route{3},previous_where20{3});
[~,previous_time1]=computeTime(previous_croute1,dis,1);
[~,previous_time2]=computeTime(previous_croute2,dis,2);
[~,previous_time3]=computeTime(previous_croute3,dis,3);
previous_time=previous_time1+previous_time2+previous_time3;
...
while 1.0 < temperature
    ...
    temp_route{3} = perturb(route{3},'reverse',dis,previous_where20{3}, 3);
    ...
    [route_new31, routenew32]=completeroute(temp_route{3},current_where20{3});
    [k11,k12]=computeTime(routenew12,dis,1);
    [k21,k22]=computeTime(routenew22,dis,2);
    [k31,k32]=computeTime(routenew32,dis,3);
    if k11 ==1 && k21==1 && k31 ==1
        ...
        if (diff < 0) || (rand < exp(-diff/(temperature)))
            ...
            [xxx,nnew_route{3}]=completeroute(route{3},previous_where20{3});
            for i =1:3
    ...
    if rand > 0.8
        ...
        [route_new31, routenew32]=completeroute(temp_route1{3},current_where20{3});
        [k11,k12]=computeTime(routenew12,dis,1);
        [k21,k22]=computeTime(routenew22,dis,2);
        [k31,k32]=computeTime(routenew32,dis,3);
        if k11~=0 && k21~=0 && k31~=0
            diff = current_distance - previous_distance;
                if (diff < 0) || (rand < exp(-diff/(temperature)))
                    route = temp_route1;
                    previous_time1=k12;
...
[xxx,new_route{3}]=completeroute(route{3},previous_where20{3});
% cnt1 = 0;
% cnt11 = 1;
% flag = 0;
...
new_route{3}
% for i =1:3
%     [x,y]=computeTime(new_route{i},dis,i)
% end
...
fprintf("第三辆车路径为:");
for i = 1:length(new_route{3})-1
    fprintf("%d->",new_route{3}(i));
end
fprintf("%d\n",new_route{3}(end));