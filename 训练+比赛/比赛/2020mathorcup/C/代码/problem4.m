...
routes = cell(49, 16); % 49个订单经过4*4种路线经过的货格
for i = 1:49
    for j = 1:4
        for k = 1:4
            [result, route] = simulatedannealing(dist,num1{i},ord{i}, dots(j), dots(k));
            results(i, 4*j+k-4) = result; % 49个订单经过4*4种路线经过的距离
...
check = [3001,3003,3010,3012]; % 4个可用的复核台
checkord = zeros(9,10);
for i=1:9
    checkord(i, :) = check(round(rand(1, 10)*3)+1);
end
[joborder, checkord, endtime, ratio] = simulatedannealing4(ptasks,checkord,check,results',ordtime); % 进行求解