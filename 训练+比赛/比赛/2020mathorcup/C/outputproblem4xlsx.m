...
for i = 1:length(ptasks)
    ...
    xlsfile{row, 5} = 0;
%     xlswrite('test.xlsx',{name},'Ques4',['B',num2str(row)]);
%     xlswrite('test.xlsx',{checktaskord},'Ques4',['C',num2str(row)]);
%     xlswrite('test.xlsx',{rawcheckagent},'Ques4',['D',num2str(row)]);
%     xlswrite('test.xlsx',{checkagent},'Ques4',['E',num2str(row)]);
%     xlswrite('test.xlsx',0,'Ques4',['F',num2str(row)]);
    row = row + 1;
...
%             xlswrite('test.xlsx', {name}, 'Ques4',['B',num2str(row)]);
%             xlswrite('test.xlsx', {taskord}, 'Ques4',['C',num2str(row)]);
%             xlswrite('test.xlsx', {raword{k}}, 'Ques4',['D',num2str(row)]);
%             xlswrite('test.xlsx', {temp_route(k)}, 'Ques4',['E',num2str(row)]);
%             xlswrite('test.xlsx', {dropnum(k)}, 'Ques4',['F',num2str(row)]);
            row = row + 1;
        end
